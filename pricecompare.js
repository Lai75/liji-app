...
function renderPriceCompare(){
  ...
  el.querySelector('#priceItemList').addEventListener('click', e=>{
    ...
    if(e.target.closest('[data-act=addPrice]')){
      const amtInput = box.querySelector('.priceAmountInput');
...
function priceItemHtml(item){
  ...
  <div class="box price-group" data-id="${item.id}">
    ...
    <div class="row" style="margin-top:8px;gap:8px;">
      ...
      <input type="text" class="mono priceAmountInput" inputmode="decimal" placeholder="价格" style="width:70px;border:1px solid var(--line);border-radius:8px;padding:6px 8px;background:var(--soft);" />
      <input type="date" class="priceDateInput" value="${todayStr()}" />
      <button class="add-btn" data-act="addPrice" style="margin-left:auto;padding:6px 12px;font-size:12px;">记一次</button>
    </div>
  </div>`;
}
